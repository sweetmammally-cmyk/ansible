...
#practice for package installation , works without ansible intell

ansible -i {path of the yaml inventory file or all} { server name } -m apt -a "name=nginx state=present update_cache=yes" 
...
```bash
ansible -i {path of the yaml inventory file or all} { server name } -m setup
```
